...
function QuizQuestion({ q, submitted, resetCount,i,setAnswered,answered }: QuizQuestionInterface) {
  ...
  if (submitted) {
    ...
    return (
      <div className="rounded-xl chat-background mx-[20px] ">
        ...
        <span className=" font-extrabold">{i + "."}</span>{' ' +q.initial.question}
        </div>
...
function QuizButtons({setSubmitted,setQuizVisible,submitted,reset,questions,answered}) {
  let ansNum=0
  if(Array.isArray(answered))
 ...
 useEffect(() => {
  ...
  return (
<div style={{zIndex:9999}} className="text-start border-2 border-white absolute chat-background w-full rounded-t-xl h-[84px]">
        <div className="flex flex-row  justify-center">
<div className="text-center rounded-full w-[200px] pb-[10px] mt-[8.5px] m-[12px] bg-white/40 text-sm  ">

<div className="pt-[10px] text-zinc-600">Answered {ansNum} out of {questions.length}</div>
...
export default function Quiz({ setQuizVisible }) {
  ...
  return (
    <ScrollArea className="text-center h-[87svh]" viewportRef={null}>
      <QuizButtons answered={answered} setSubmitted={setSubmitted} setQuizVisible={setQuizVisible} submitted={submitted} reset={reset} questions={questions}/>
